    ...
    <div className="min-h-screen bg-bg">
      ...
      <div className="flex items-center justify-center p-4 sm:p-6 py-12">
        <div className="w-full max-w-125 bg-surface rounded-lg shadow-lg p-8 sm:p-10 animate-fade-in-up">
          ...
          {!file ? (
            <div onClick={() => inputRef.current?.click()} onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave} className={`group flex flex-col items-center justify-center gap-3 p-8 sm:p-10 rounded-md cursor-pointer transition-all duration-200 border-2 border-dashed ${dragging ? 'border-primary bg-surface-2' : 'border-border bg-bg'}`}>
              ...
              <div className="flex flex-wrap gap-2 justify-center mt-1">
                {['PDF', 'JPG', 'PNG', 'Max 10 MB'].map((tag) => (
                  <span
                    key={tag}
                    className="text-[11px] bg-surface-2 text-primary-dark border border-border font-semibold px-3 py-1 rounded-full"
                  >
                    {tag}
                  </span>
                ))}
